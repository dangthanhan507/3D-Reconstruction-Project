...
def prune_Triangle(tri,threshold=2):
    copy_tri = tri.points[tri.simplices.copy(),:]
    swap1_tri = tri.points[tri.simplices.copy(),:]
    swap2_tri = tri.points[tri.simplices.copy(),:]
    
    #copy tri
    # row 0, row 1, row 2
    
    #swap 1
    # row 0 , row 1, row 2 -> swap
    # row 2, row 1, row 0 -> swap
    
    # row 2, row 0, row 1
    swap1_tri[:,[0,2],:] = swap1_tri[:,[2,0],:] # swap row 1 and row 3
    swap1_tri[:,[1,2],:] = swap1_tri[:,[2,1],:] # swap row 2 and row 3
    
    #swap2
    # row 0, row 1, row 2 -> swap
    # row 1, row 0, row 2 -> swap
    
    # row 1, row 2, row 0
    swap2_tri[:,[0,1],:] = swap2_tri[:,[1,0],:] # swap row 0 and row 1
    swap2_tri[:,[2,1],:] = swap2_tri[:,[1,2],:] # swap row 2 and row 1
    
    diff1 = np.linalg.norm(copy_tri - swap1_tri,axis=2) # reduce to 2d matrix
    diff2 = np.linalg.norm(copy_tri - swap2_tri,axis=2)
    
    diff1_bool = (diff1 > threshold)
    diff2_bool = (diff2 > threshold)
    diff_bool = diff1_bool | diff2_bool
    maskSum = np.sum(diff_bool,axis=1)
    
    assert maskSum.shape[0] == tri.simplices.shape[0]
    
    mask = (maskSum == 0) # find out which ones had valid edges
    prune_tri = tri.simplices[mask,:]
    unique_indexes = np.unique(prune_tri) # remaining indexes that are good 
    
    return unique_indexes, prune_tri
...
if __name__ == '__main__':
    ...
    for i in range(len(pts3_total)):
        pts3 = pts3_total[i]
        colormap = colormap_total[i]/255

        xx,yy = np.meshgrid( np.arange(im_frame.shape[1]), np.arange(im_frame.shape[0]) )
        pts2 = np.vstack((xx.flatten(),yy.flatten()))[:,masks[i].flatten()]
        tri2 = Delaunay(pts2.T)
        unique_indexes,tri = prune_Triangle(tri2)
        writeply(pts3,colormap.T,tri,f'meshes/mesh{i}.ply')
